...
export default function WalletRow({ entry }: WalletRowProps) {
  ...
  return (
    <tr className="border-b border-zinc-800/60 transition-colors hover:bg-zinc-800/30">
      <td className="px-4 py-3 text-sm font-medium tabular-nums text-zinc-400">#{entry.rank}</td>
      <td className="px-4 py-3">
        <div className="flex items-center gap-2">
          <span className="font-mono text-sm text-zinc-200">{shortenAddress(entry.wallet_address)}</span>
          <button
            type="button"
            onClick={copy}
      ...
      <td className="px-4 py-3 text-right text-sm tabular-nums text-zinc-400">{entry.total_tx.toLocaleString()}</td>
    </tr>
  );
}
