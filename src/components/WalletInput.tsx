...
import { Check, Copy, ExternalLink, Eye, EyeOff, Share2, Sparkles, X } from 'lucide-react';
import { useEffect, useState } from 'react';
import { Button } from '@/components/ui/button';
import { Card, CardContent, CardDescription, CardHeader, CardTitle } from '@/components/ui/card';
import { Input } from '@/components/ui/input';
import { resolveENSToAddress } from '@/services/ens';
import { fetchRecentRequests, isValidEthereumAddress } from '@/services/relayApi';
...
export default function WalletInput({ onAnalyze, onReset, isLoading, analyzedAddress, ensName }: WalletInputProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Check if it's an ENS name (.eth), including Base names like .base.eth
  ...
  return (
    <div className="relative group w-full mx-auto">
      ...
      <div
        className="absolute -inset-[2px] opacity-0 group-hover:opacity-100 group-focus-within:opacity-100 transition-opacity duration-300 overflow-hidden"
      ...
      <Card className="relative w-full border border-border/50 shadow-2xl bg-card transition-all duration-300 group-hover:shadow-[0_0_40px_rgba(139,92,246,0.2)] group-focus-within:shadow-[0_0_40px_rgba(139,92,246,0.2)]">
        <CardHeader className="space-y-3 sm:space-y-4 pb-4 sm:pb-6">
          ...
          <CardDescription className="text-sm sm:text-base text-muted-foreground leading-relaxed">
            Discover your cross-chain journey with Relay Protocol. Enter any wallet address or ENS name to unlock
